    ...
    public class Story:Repository<CI.Models.Story>,IStory
    {
        ...
        public Models.ViewModels.Mission GetSearchStory(string key)
        {
            ...
            var storys = (from s in stories
                          ...
                          select s).ToList();
            var story = new Models.ViewModels.Mission
            {
                Stories = storys,
            };
        ...
        public bool Recommend(long user_id, long story_id, List<long> co_workers)
        {
            ...
            {
                var notificationsettingrecord = _db.NotificationSettings.FirstOrDefault(x => x.UserId == user);
                var fromrecord = _db.Users.FirstOrDefault(user => user.UserId == user_id);
                var storyrecord = _db.Stories.FirstOrDefault(story => story.StoryId == story_id);
                if (notificationsettingrecord.RecommendStory == 1)
                {
                    Notification notification = new Notification();
                    notification.NotificationText = fromrecord.FirstName + " " + fromrecord.LastName + " - Recommends this story - " + storyrecord.Title;
                    notification.NotificationType = 0;
                    notification.FromUserId = fromrecord.UserId;
                    notification.StoryId= story_id;
